...
class UploadModelHelper
  ...
  def self.cast_upload_results(results)
    ...
    results.each { |r|
      #puts r
      #puts r['upid']
      upload = Uploadmodel.new(r['uploadtype'], r['file_name'], r['original_file_name'], r['username'], r['title'], r['description'], r['upload_datetime'], r['upid'], r['overallscore'], r['numofratings'], r['averagescore'], r['email'])
      ...
      upload.tags.each { |tag|
        ...
        tags_rank[tag.tagName] = leaderboard.to_a.index { |x| x['id'] == r['upid'] } + 1
      }
